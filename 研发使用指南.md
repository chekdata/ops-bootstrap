...
- CI 门禁对齐
 ...
 - 平台账号与治理（Nacos / YApi / Ingress‑NGINX）
  ...
  - YApi：
    ...
    - 分组与成员（统一四个分组，仅此四个）：`Miker`、`app`、`SaaS`、`公用微服务`。
      - 飞连 SSO 登录后，用户会自动加入以上四个分组（无需手动加组）。
    - 平台非生产统一实例（dev/staging 共用）：不新装新环境，按项目“开分区”。
      - 命名：`<product>-<env>`（例：`miker-dev`）。
      - 操作：在对应产品分组（如 `Miker`）中“新建项目=<product>-<env>`”，CI 配 `YAPI_TOKEN_DEV`，构建后用可复用工作流导入 OpenAPI。
  - Ingress‑NGINX：
    ...
    - TLS 策略：仅生产环境启用 TLS（由 cert-manager/ClusterIssuer 自动签发）；dev/staging 默认不启用证书（HTTP），如需临时启用请在项目内显式覆盖 values。
...
- 现状/资源
  ...
  - API Server 公网访问
    - chek-prod-k8s：IPv4 115.190.177.69
    - chek-dev-k8s：IPv4 101.126.47.206
    - chek-staging-k8s：IPv4 115.190.123.201
...
（新增）部署环境接入 TOS 常见坑与实战
- 节点出网受限导致 Pod 卡在 ContainerCreating：
  - 现象：无法拉取 `registry.k8s.io/pause:3.6`，报 `us-west2-docker.pkg.dev` 超时。
  - 解决：已落地 DaemonSet `kube-system/pause-fix`，在所有节点上强制 kubelet/containerd 使用国内可达或 VECR 镜像（现为 `chek-images-cn-beijing.cr.volces.com/prod/pause:3.6`）。
- containerd 配置重复表导致无法重启：
  - 现象：`containerd: failed to load TOML duplicated tables`。
  - 解决：重写最小配置 `version=2` 并仅注入 `plugins."io.containerd.grpc.v1.cri".sandbox_image`。
- TOS 桶区域与端点不一致：
...
- 自检清单（过线即对外联调）
  ...
  - Service targetPort=容器端口，Ingress backend 使用 `port.name: http`；健康探针通过。
...
- 端口与路由（强制）
  ...
  - `Service` 端口命名 `http`，`targetPort` 指向容器端口；`Ingress` backend 使用 `port.name: http`。
- 必需环境变量
...
- 仓库侧接入（两种方式，二选一）
  ...
  - 本地生成版（稳定，服务不直接暴露 OpenAPI 时）
    1) 在 CI 构建阶段生成 `build/swagger/swagger.json`（按技术栈选择一条）：
       - Spring Boot（springdoc-openapi）：`mvn -Popenapi generate` 或 Gradle 插件导出至构建目录。
    ...
    2) 在业务仓库新增 `.github/workflows/yapi-sync.yml`：
       ```yaml
       name: yapi-sync
       on:
         push:
...
- 工作流位置：仓库复用 `/Users/jasonhong/Desktop/CICD/.github/workflows/yapi-sync-reusable.yml`（可复用 workflow）。
