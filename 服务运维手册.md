...
- 工作流概览（GitHub Actions）：
  ...
  - `deploy-helm-prod`（`.github/workflows/deploy-prod.yml`）：
    ...
    - 行为：使用 `KUBECONFIG_PROD` 在 `saas-prod` 命名空间执行 prod 部署，镜像标签为该 tag（`vX.Y.Z`），Ingress host 固定 `api.chekkk.com`。
...
- **Prod（`saas-prod`）**：
  ...
  - `deploy-helm-prod` 已按规范配置为 `vX.Y.Z` tag 触发、host 固定 `api.chekkk.com`，本轮尚未通过真实 tag 发布做一次完整演练，可视为“配置就绪、待一次实战验证”。
- **OpenAPI / YApi / LLM**：
