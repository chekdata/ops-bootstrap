...
class DataProcessor:
    ...
    def async_process(self, csv_file: str = None):
        ...
        #     self.executor.submit(partial(self.process_state, self.shared_data)),
        ...
        future_gps_data = self.executor.submit(partial(self.process_gps_data, self.shared_data))
        future_gps_data.result()

        future_state = self.executor.submit(partial(self.process_state, self.shared_data))
        future_state.result()

        future_speed = self.executor.submit(partial(self.process_speed, self.shared_data))
        future_speed.result()

        future_acc = self.executor.submit(partial(self.process_acc, self.shared_data))
        future_acc.result()
    ...
    def process_time(self, shared_data):
        # drop invalid time
        start_t = None
    ...
    @staticmethod
    def process_gps_data(shared_data):
        df = pd.DataFrame.from_dict(shared_data['df'])
        df_gps_timestamp = df['gps_timestamp'].copy()
        df_gps_speed = df['gps_speed'].copy()
        df_lon = df['lon'].copy()
        df_lat = df['lat'].copy()
        for i, (t, gps_v, lon, lat) in enumerate(zip(df['gps_timestamp'], df['gps_speed'], df['lon'], df['lat'])):
            future_i, delta_t = DataProcessor.get_idx_after_seconds(df, i, 1.5)
        ...
        df['gps_timestamp'] = df_gps_timestamp
        df['gps_speed'] = df_gps_speed
        df['lon'] = df_lon
        df['lat'] = df_lat

        shared_data['df'] = df.to_dict()

    # @staticmethod
    # def process_speed(shared_data):

    #     df = pd.DataFrame.from_dict(shared_data['df'])

    #     df_speed = df['speed'].copy()
    #     df.insert(4, 'speed_old', df_speed)
    #     next_valid_idx = -1
    #     pre_valid_v, pre_valid_gps_v = None, None
    #     # gps speed has about 1s delay
    #     for i, v in enumerate(df['speed']):
    #         gps_v = df['gps_speed'][DataProcessor.get_idx_after_seconds(df, i, 1)[0]]
    #         is_valid_gps = DataProcessor.is_valid_gps(df, i)
    #         # When gps is valid
    #         if is_valid_gps:
    ...
    #             next_valid_idx = DataProcessor.get_next_valid_speed_idx(df, i)
    #         expected_v = None
    #         if next_valid_idx is None:
    #             expected_v = df_speed[max(0, i-1)]
    #         else:
    #             pre_v, pre_t = df_speed[max(
    #                 0, i-1)], df['time'][max(0, i-1)]
    #             next_v, next_t = df_speed[next_valid_idx], df['time'][next_valid_idx]
    #             if i == 0:
    #                 expected_v = next_v
    #             else:
    #                 p = (df['time'][i] - pre_t) / (next_t - pre_t)
    ...
    def process_speed(shared_data):
        ...
        df_gps_speed = df['gps_speed'].values
    ...
    def process_acc(shared_data):
        ...
        for i, v in enumerate(df_speed):
            future_1s_idx, delta_t = DataProcessor.get_idx_after_seconds(df, i, 1)
            nv = df_speed[future_1s_idx]
            acc = (nv-v)/3.6/delta_t if delta_t > 0 else last_acc
            last_acc = acc
            df_acc[i] = acc
            # df_acc[i] = a if abs(a) < 8 else 0
        #df.insert(6, 'acc', df_acc)
        df['acc'] = df_acc

        shared_data['df'] = df.to_dict()

    @staticmethod
    def process_state(shared_data):

        df = pd.DataFrame.from_dict(shared_data['df'])

        df_state = df['auto_car'].copy()
        for i, (auto_icon, auto_car) in enumerate(zip(df['auto_icon'], df['auto_car'])):
            ...
            if pd.isna(auto_icon) and not pd.isna(auto_car):
                ...
                continue
            # consider None
            future_7s_idx, delta_t = DataProcessor.get_idx_after_seconds(df, i, 7)
            auto_icon_counts = df['auto_icon'][i +
                                                    1: future_7s_idx].value_counts(dropna=False)
            auto_car_counts = df['auto_car'][i +
                                                  1: future_7s_idx].value_counts(dropna=False)
        ...
        df.insert(4, 'state_no_smooth', df_state)
        # smooth state
        ...
        for i, state in enumerate(df_state):
            ...
            if state != in_state:
                ...
                if in_state_seconds < 3:
                    future_3s_idx, _ = DataProcessor.get_idx_after_seconds(df, i, 3)
    ...
    @staticmethod
    def is_valid_speed(df, current_idx):
        v = df['speed'][current_idx]
        past_3s_i, delta_t = DataProcessor.get_idx_before_seconds(df, current_idx, 3)
        future_3s_i, delta_t = DataProcessor.get_idx_after_seconds(df, current_idx, 3)
        v_median = df['speed'][past_3s_i: future_3s_i].median()
        v_mean = df['speed'][past_3s_i: future_3s_i].mean()
        if not pd.isna(v) and abs(v_mean - v) < 10 and abs(v_median - v) < 10:
            return True
        return False

    @staticmethod
    def get_next_valid_speed_idx(df, current_idx):
        future_i = current_idx + 1
        # try through valid gps
        future_10s_i, delta_t = DataProcessor.get_idx_after_seconds(df, current_idx, 10)
        while future_i < len(df) and future_i <= future_10s_i:
            # future 10s
            v = df['speed'][future_i]
            gps_v = df['gps_speed'][DataProcessor.get_idx_after_seconds(
                df, future_i, 1)[0]]
            if DataProcessor.is_valid_gps(df, future_i) and not pd.isna(v) and abs(gps_v - v) < 10:
                return future_i
            future_i += 1
        # try other
        future_i = current_idx + 1
        while future_i < len(df):
            if DataProcessor.is_valid_speed(df, future_i):
                return future_i
            future_i += 1
        return None
    ...
    @staticmethod
    def get_idx_after_seconds(df, current_idx, seconds):
        current_t = df['time'][current_idx]
        delta_t = 0
        future_idx = current_idx
        while future_idx < len(df) and delta_t < seconds:
            delta_t = df['time'][future_idx] - current_t
            future_idx += 1
        return future_idx-1, delta_t

    @staticmethod
    def get_idx_before_seconds(df, current_idx, seconds):
        current_t = df['time'][current_idx]
        delta_t = 0
        past_idx = current_idx
        while past_idx > 0 and delta_t < seconds:
            delta_t = current_t - df['time'][past_idx]
            past_idx -= 1
        return past_idx+1, delta_t

    @staticmethod
    def get_idx_by_frame_id( df, frame_id):
        l_idx, r_idx = 0, len(df)-1
        while l_idx <= r_idx:
            m_idx = l_idx+int((r_idx-l_idx)/2)
            m_frame_id = df['frame'][m_idx]
    ...
    @staticmethod
    def is_valid_gps(df, current_idx):
        past_idx, _ = DataProcessor.get_idx_before_seconds(df, current_idx, 5)
        future_idx, _ = DataProcessor.get_idx_after_seconds(df, current_idx, 5)
        recent_gps_speed_df = df['gps_speed'][past_idx:future_idx]
        for i in range(past_idx, future_idx-1):
            ...
            if pd.isna(p_gps_speed) or abs(p_gps_speed-n_gps_speed) > 40:
                return False
            future_3s_idx, delta_t = DataProcessor.get_idx_after_seconds(df, i, 3)
            if delta_t >= 3 and df['gps_timestamp'][future_3s_idx] == df['gps_timestamp'][i]:
                return False
        return True
